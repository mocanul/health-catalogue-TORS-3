...
export default function Booking({ selectedRoom, onRoomSelect, bookingItems, onRemoveItem, clearItems }: Props) {
    ...
    return (
        <div className="border-2 border-gray-100 border-r flex-1 flex items-center justify-center m-8 min-w-0 overflow-hidden">
                        ...
                        <div className="flex flex-col gap-2 overflow-y-auto h-48 max-h-48 pr-1">
                            {bookingItems.length > 0 ? (
                                bookingItems.map((item) => (
                                    ...
                                    >
                                        <p className="text-xs font-medium text-gray-800 truncate flex-1 min-w-0">{item.name}</p>
                                        <div className="flex items-center gap-1.5 mx-3 shrink-0">
                                            <p className="text-xs text-gray-700">Q:</p>
                                            <span className="text-xs w-4 text-center text-gray-700">{item.quantity}</span>
                                        </div>
                                        <button
                                ...
                                <p className="text-xs text-gray-300 italic text-center py-4">No items added yet</p>
                            )}
                        </div>
            ...
            <SelectRoom
                ...
                onSelect={(room) => onRoomSelect(room)}
            />
