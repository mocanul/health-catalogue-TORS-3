...
jest.mock("@/lib/mail", () => ({
    sendRequestedPasswordSetupEmail: jest.fn(),
}))
...
const { createPasswordSetupToken: mockCreatePasswordSetupToken } = jest.requireMock(
    "@/lib/auth/passwordSetup"
) as {
    createPasswordSetupToken: jest.Mock
}

const { sendRequestedPasswordSetupEmail: mockSendRequestedPasswordSetupEmail } =
    jest.requireMock("@/lib/mail") as {
        sendRequestedPasswordSetupEmail: jest.Mock
    }

import { POST } from "@/app/api/auth/first-login/request-pass/route"


describe("POST /api/auth/first-login/request-pass", () => {
    beforeEach(() => {
        jest.clearAllMocks()
    ...
    it("returns 200 and sends setup email for valid user", async () => {
        ...
        mockSendRequestedPasswordSetupEmail.mockResolvedValue(undefined)
        ...
        expect(mockSendRequestedPasswordSetupEmail).toHaveBeenCalledWith({
            email: "user@example.com",
            firstName: "Ada",
            setupLink:
